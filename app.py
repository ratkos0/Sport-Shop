...
#KONFIG MYSQLA (POTREBAN UNOS VALIDNIH PODATAKA OD BAZE)
...
class RegisterForm(FlaskForm):
    FirstName = StringField('Unesite Vase ime:', validators=[DataRequired(), Length(max=20)])
...
class LoginForm(FlaskForm):
    username = StringField('Unesite korisničko ime:', validators=[DataRequired(), Length(max=20)])
...
def register():
    ...
    if request.method == 'POST':
        ...
        if pswrd==verify_pass:
            #Provjerava postoji li email i usernameu bazi podataka, ako nema komituje
